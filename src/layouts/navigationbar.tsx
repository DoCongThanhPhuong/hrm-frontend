      ...
      {navigationItems.map(({ Icon, title, path, update }, index) => {
        ...
        return (
          <Link
            ...
            href={path}
            prefetch
            className={`group relative z-0 rounded-xl hover:bg-neutral1-5 active:bg-neutral4-30 ${
              expanded ? 'w-full' : 'w-fit'
            } ${isActive && activeStyle}`}
          >
            <div
              className={`group/sub group-[.active]:bg-neutral2-10 group-[.active]:hover:bg-neutral2-15 group-[.active]:active:bg-neutral4-30 group-[.active]:p-[5px] hover:bg-neutral1-5 active:neutral4-30 flex items-center gap-3 p-1.5 rounded-[11px] backdrop-blur-16 relative z-1 ${
                expanded ? '' : 'w-fit'
              }`}
            >
