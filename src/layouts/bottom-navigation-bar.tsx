...
const BottomNavItem = ({ navItem }: BottomNavItemProps) => {
  ...
  return (
    <button
      ...
      className={` relative z-0  flex items-center justify-center rounded-xl ${
        isActive &&
        `active p-[1px] bg-neutral2-10 rounded-xl before:absolute before:inset-0 before:opacity-15 before:bg-linear-card before:rounded-xl after:content-[''] after:absolute after:inset-[1px] after:bg-[#313131] after:rounded-[11px] `
      }`}
    >
      <div
